...
const Card = ({ name, username, id, remove }) => {
  ...
  const actionFav = () => {
    ...
    if(pathname === "/favs"){
      remove(id)
      alert("El odontologo se elimino de tus favoritos")
      return
    }
    ...
    if(!localStorage.getItem("favorites")){
      ...
      alert("El odontologo fue agregado a tus favoritos")
    } else{
      ...
      if(favoriteDentistIndex >= 0){
        ...
        alert("El odontologo ya se encuentra agregado a tus favoritos")
      }else{
  ...
  return (
    ...
    <div className="card">
      ...
      <button onClick={actionFav} className="favButton">{pathname === "/home" ? '⭐' : '🌟'}</button>
    </div>
  );
};
